...
portfolios <- left_join(indices_equal, efficient_index, by = "date") %>% 
  gather(index, value, eq_weight:efficient)
...
# Combination Graph
ggplot() + 
  geom_line(data = portfolios, aes(x=date, y=value, color = index)) + 
...
The graph shows clearly that the most efficient portfolio is one in which the heaviest weighting is placed on the precious metals component of the portfolio, which is not surprising given it's outperformance of the industrial metals class, especially over the last ten years. 
