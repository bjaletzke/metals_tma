...
eq_w_ind <- combined_indices %>% 
  mutate(eq_ind = eq_w_pm * precious_metals + eq_w_ind * industrial_metals) %>% 
  subset(select = c(date, eq_ind))

# join with main dataframe
combined_indices_join <- right_join(eq_w_ind, combined_indices, by = "date") %>% 
 # subset(select = -c(ind_close, pm_close )) %>% 
  gather(index, value, eq_ind:precious_metals)

# Index plot vs. Separate Lines
ggplot() + 
  geom_line(data = combined_indices, aes(x=date, y=eq_ind, color = 
```
